...
class MarketConditionsProvider:
    ...
    async def get_market_conditions(
        ...
        if (
            not force_refresh
            and self.cached_conditions
            and self.last_update
            and (now - self.last_update).total_seconds()
                ...
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        if "bitcoin" in data:
                            ...
                            conditions.btc_24h_change = data["bitcoin"].get(
                                "usd_24h_change", 0.0
                            )

                        if "ethereum" in data:
                            conditions.eth_price = data["ethereum"]["usd"]
                            conditions.eth_24h_change = data["ethereum"].get(
                                "usd_24h_change", 0.0
                            )
                ...
                async with session.get(global_url) as response:
                    if response.status == 200:
                        ...
                        if "data" in data:
                            ...
                            conditions.btc_dominance = global_data.get(
                                "market_cap_percentage", {}
                            ).get("btc", 0.0)
                            conditions.total_market_cap = global_data.get(
                                "total_market_cap", {}
                            ).get("usd", 0.0)
                            conditions.market_cap_24h_change = global_data.get(
                ...
                for symbol in ["BTCUSDT", "ETHUSDT"]:
                    ...
                    async with session.get(funding_url, params=params) as response:
                        if response.status == 200:
                            data = await response.json()
                            funding_rate = (
                                float(data.get("lastFundingRate", 0)) * 100
                            )  # Convert to %

                            if symbol == "BTCUSDT":
                                conditions.btc_funding_rate = (
                                    funding_rate * 365 * 3
                                )  # Annualized
                            else:
                ...
                for symbol in ["BTCUSDT", "ETHUSDT"]:
                    ...
                    async with session.get(depth_url, params=params) as response:
                        if response.status == 200:
                            ...
                            if current_price > 0:
                                depth = self._calculate_orderbook_depth(
                ...
                async with session.get(self.fear_greed_api) as response:
                    if response.status == 200:
                        ...
                        if "data" in data and len(data["data"]) > 0:
                            conditions.crypto_fear_greed_index = int(
                                data["data"][0]["value"]
                            )
    ...
    async def _calculate_volatility_metrics(self, conditions: MarketConditions):
        ...
        try:
            ...
            conditions.vol_risk_premium_btc = (
                conditions.btc_implied_vol - conditions.btc_realized_vol_7d
            )
            conditions.vol_risk_premium_eth = (
                conditions.eth_implied_vol - conditions.eth_realized_vol_7d
            )

        except Exception as e:
            logger.warning(f"Volatility metrics calculation failed: {e}")

    def _calculate_orderbook_depth(
        self, orderbook_data: Dict, current_price: float, depth_pct: float
    ) -> float:
        ...
        try:
            ...
            bid_depth = sum(
                float(qty) for price, qty in bids if float(price) >= min_price
            )
            ask_depth = sum(
                float(qty) for price, qty in asks if float(price) <= max_price
            )
    ...
    def _determine_market_regime(self, conditions: MarketConditions):
        ...
        # Funding environment
        avg_funding = (
            abs(conditions.btc_funding_rate) + abs(conditions.eth_funding_rate)
        ) / 2
