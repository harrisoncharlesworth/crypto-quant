...
class SignalAttributionTracker:
    ...
    def __init__(self, max_history_days: int = 90):
        ...
        self.daily_pnl: Dict[str, Dict[str, float]] = defaultdict(dict)  # date -> signal -> pnl
        
        # Initialize performance tracking for known signals
        self.known_signals = [
            "time_series_momentum", "donchian_breakout", "short_term_mean_reversion",
            "oi_price_divergence", "delta_skew_whipsaw", "perp_funding_carry",
            "alt_btc_cross_sectional", "cash_carry_basis", "cross_exchange_funding",
            "options_vol_risk_premium", "stablecoin_supply_ratio", "mvrv_zscore"
        ]
        
        for signal in self.known_signals:
            self.signal_performance[signal] = SignalPerformance(signal_name=signal)
        
        logger.info(f"Signal Attribution Tracker initialized with {len(self.known_signals)} signals")
    ...
    def record_trade_entry(
        ...
        if signal_name not in self.signal_performance:
            self.signal_performance[signal_name] = SignalPerformance(signal_name=signal_name)
    ...
    def record_trade_exit(
        ...
        logger.debug(f"Recorded trade exit: {signal_name} on {symbol}, P&L: ${total_pnl:.2f}")
        return True
    ...
    def _update_signal_performance(self, trade: TradeRecord):
        ...
        signal_perf.hit_rate = (signal_perf.winning_trades / signal_perf.total_trades 
                               if signal_perf.total_trades > 0 else 0.0)
        
        signal_perf.average_pnl_per_trade = (signal_perf.total_pnl / signal_perf.total_trades 
                                           if signal_perf.total_trades > 0 else 0.0)
        
        # Calculate hold time
        if trade.exit_timestamp and trade.timestamp:
            hold_time = (trade.exit_timestamp - trade.timestamp).total_seconds() / 3600
            current_avg = signal_perf.avg_hold_time_hours * (signal_perf.total_trades - 1)
            signal_perf.avg_hold_time_hours = (current_avg + hold_time) / signal_perf.total_trades
        
        # Track confidence vs performance
        confidence_bucket = f"{int(trade.signal_confidence * 10) * 10}%"  # 0%, 10%, 20%, etc.
    ...
    def get_signal_attribution_report(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_trades = [t for t in self.trade_records 
                        if t.timestamp >= cutoff_date and t.is_closed]
        ...
        total_portfolio_pnl = sum(attr["total_pnl"] for attr in signal_attribution.values())
        if total_portfolio_pnl != 0:
            for attr in signal_attribution.values():
                attr["contribution_pct"] = (attr["total_pnl"] / total_portfolio_pnl) * 100
        
        # Top contributors and detractors
        sorted_signals = sorted(signal_attribution.items(), 
        ...
        return {
            ...
            "overall_hit_rate": (sum(1 for t in recent_trades if t.pnl and t.pnl > 0) / 
    ...
    def get_sector_attribution(self, days: int = 30) -> Dict[str, float]:
        ...
        sector_map = {
            ...
            "BNB": "exchange", "SOL": "layer1", "ADA": "layer1",
            "DOT": "interoperability", "AVAX": "layer1", "LINK": "oracle",
            "UNI": "defi", "DOGE": "meme", "SHIB": "meme",
    ...
    def get_trade_quality_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        total_notional = sum(abs(t.position_size * t.entry_price) for t in recent_trades)
        
        avg_slippage_bps = (total_slippage / total_notional * 10000) if total_notional > 0 else 0
        fee_rate_bps = (total_fees / total_notional * 10000) if total_notional > 0 else 0
        ...
        return {
            ...
            "pnl_per_dollar_traded": (sum(t.pnl or 0 for t in recent_trades) / total_notional 
    ...
    def cleanup_old_records(self):
        ...
        self.trade_records = [t for t in self.trade_records if t.timestamp >= cutoff_date]
