...
async def test_signal_without_auth():
    ...
    try:
        ...
        print(f"   Direction: {'LONG' if result.value > 0 else 'SHORT' if result.value < 0 else 'NEUTRAL'}")
        
        if result.metadata:
            print(f"   Momentum Return: {result.metadata.get('momentum_return', 0):.3f}")
            print(f"   Price vs MA: {result.metadata.get('price_vs_ma', 0):.3f}")
        
        print(f"\n🎯 Signal Quality Check:")
        print(f"   Value in range [-1, 1]: {'✅' if -1 <= result.value <= 1 else '❌'}")
        print(f"   Confidence in range [0, 1]: {'✅' if 0 <= result.confidence <= 1 else '❌'}")
...
async def main():
    ...
    if success:
        ...
        print(f"\n📋 Next steps:")
        print(f"   1. Fix Binance API authentication (check trading permissions)")
        print(f"   2. Set up email credentials for notifications")
        print(f"   3. Run full system test")
