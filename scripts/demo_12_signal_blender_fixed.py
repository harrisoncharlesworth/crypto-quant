...
class MockSignalGenerator:
    ...
    def _generate_price_series(self, length: int = 1000) -> pd.Series:
        ...
        returns[::50] += np.random.normal(
            0, 0.1, len(returns[::50])
    ...
    def get_current_price(self) -> float:
        ...
        return self.price_history.iloc[
            min(self.current_idx, len(self.price_history) - 1)
        ]
    ...
    def generate_all_signals(self, symbol: str = "BTCUSDT") -> Dict[str, SignalResult]:
        ...
        prev_prices = self.price_history.iloc[
            max(0, self.current_idx - 30) : self.current_idx + 1
        ]
        ...
        signals["donchian_breakout"] = SignalResult(
            ...
            timestamp=datetime.utcnow(),
            metadata={"channel_period": 55, "realized_vol": 0.15, "atr_multiple": 2.0},
        )
        ...
        price_momentum = (
            (prev_prices.iloc[-1] / prev_prices.iloc[-5] - 1)
        ...
        skew_signal = (
            -np.sign(skew) * min(abs(skew * 4), 0.6) if abs(skew) > 0.15 else 0
        )
        signals["delta_skew_whipsaw"] = SignalResult(
            ...
            timestamp=datetime.utcnow(),
            metadata={"skew_25d": skew, "skew_threshold": 0.15, "realized_vol": 0.09},
        )
        ...
        signals["mvrv_zscore"] = SignalResult(
            ...
            metadata={
                ...
                "regime": (
                    "euphoric"
                    if mvrv_zscore > 7
                    else "value" if mvrv_zscore < -1 else "neutral"
                ),
                "signal_type": "overlay",
                "realized_vol": 0.02,
            },
...
def run_blender_demo():
    ...
    for method in allocation_methods:
        ...
        config = BlenderConfigV2(
            ...
            risk_limits=RiskLimits(
                max_net_exposure=0.30, max_gross_leverage=2.5, max_single_position=0.10
            ),
        )
        ...
        for step in range(100):
            ...
            if (step + 1) % 20 == 0:
                print(
                    f"   Step {step + 1}/100: Net={result.final_position:.3f}, "
        ...
        print(
            f"   └─ Max Correlation: {final_result.risk_metrics['max_correlation']:.3f}"
        )
        ...
        if violations:
            ...
            for violation in violations:
                print(
                    f"      - {violation['type']}: {violation['current']:.3f} > {violation['limit']:.3f}"
                )
        else:
    ...
    sorted_contributions = sorted(
        final_result.signal_contributions.items(), key=lambda x: abs(x[1]), reverse=True
    )
    for signal_name, contribution in sorted_contributions[:8]:  # Top 8
        signal_type = config.signal_classifications.get(
    ...
    print(
        f"└─ Confidence-Weighted Exposure: {risk_metrics['confidence_weighted_exposure']:.4f}"
    )
    ...
    for signal_type, exposure in type_exposures.items():
        ...
        print(
            f"├─ {signal_type.value:<15}: {exposure:.4f} ({utilization:.1%} of limit)"
        )
    ...
    if correlation_risks:
        ...
        for pair, risk in sorted(
            correlation_risks.items(), key=lambda x: x[1], reverse=True
        )[:5]:
