...
def check_git_files():
    ...
    if issues_found:
        ...
        for issue in issues_found:
            print(
                f"   {issue['file']}:{issue['line']} - {issue['pattern']} - {issue['match']}"
            )
...
def check_env_files():
    ...
    for env_file in env_files:
        if os.path.exists(env_file):
            ...
            try:
                result = subprocess.run(
                    ["git", "ls-files", env_file], capture_output=True, text=True
                )
...
def main():
    ...
    checks = [check_git_files(), check_gitignore(), check_env_files()]
